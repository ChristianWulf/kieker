...
public class TestGeneratedBeforeOperationEvent extends AbstractGeneratedKiekerTest {
	...
	public TestGeneratedBeforeOperationEvent() {
	...
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			...
			// check values
			Assert.assertEquals("BeforeOperationEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("BeforeOperationEvent.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("BeforeOperationEvent.orderIndex values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getOrderIndex());
