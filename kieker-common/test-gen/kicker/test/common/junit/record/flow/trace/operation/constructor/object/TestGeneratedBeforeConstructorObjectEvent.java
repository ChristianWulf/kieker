...
import kicker.common.record.flow.trace.operation.constructor.object.BeforeConstructorObjectEvent;
...
public class TestGeneratedBeforeConstructorObjectEvent extends AbstractGeneratedKiekerTest {
	...
	public TestGeneratedBeforeConstructorObjectEvent() {
	...
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			...
			// check values
			Assert.assertEquals("BeforeConstructorObjectEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("BeforeConstructorObjectEvent.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("BeforeConstructorObjectEvent.orderIndex values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getOrderIndex());
			Assert.assertEquals("BeforeConstructorObjectEvent.operationSignature values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?"":STRING_VALUES.get(i % STRING_VALUES.size()), record.getOperationSignature());
