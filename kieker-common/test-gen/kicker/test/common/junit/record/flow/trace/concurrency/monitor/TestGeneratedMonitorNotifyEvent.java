...
import kicker.common.record.flow.trace.concurrency.monitor.MonitorNotifyEvent;
 ...
 * @since 1.8
 */
public class TestGeneratedMonitorNotifyEvent extends AbstractGeneratedKiekerTest {

	public TestGeneratedMonitorNotifyEvent() {
		// empty default constructor
	}

	/**
	 * Tests {@link MonitorNotifyEvent#TestMonitorNotifyEvent(long, long, int, int)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			MonitorNotifyEvent record = new MonitorNotifyEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
			
			// check values
			Assert.assertEquals("MonitorNotifyEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("MonitorNotifyEvent.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("MonitorNotifyEvent.orderIndex values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getOrderIndex());
			Assert.assertEquals("MonitorNotifyEvent.lockId values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getLockId());
	...
	public void testEquality() {
		int i = 0;
		MonitorNotifyEvent oneRecord = new MonitorNotifyEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
		i = 0;
		MonitorNotifyEvent copiedRecord = new MonitorNotifyEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
	...
	public void testUnequality() {
		...
		MonitorNotifyEvent oneRecord = new MonitorNotifyEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
		i = 2;
		MonitorNotifyEvent anotherRecord = new MonitorNotifyEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
