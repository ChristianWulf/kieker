...
import kicker.common.record.flow.trace.concurrency.monitor.MonitorNotifyAllEvent;
 ...
 * @since 1.8
 */
public class TestGeneratedMonitorNotifyAllEvent extends AbstractGeneratedKiekerTest {

	public TestGeneratedMonitorNotifyAllEvent() {
		// empty default constructor
	}

	/**
	 * Tests {@link MonitorNotifyAllEvent#TestMonitorNotifyAllEvent(long, long, int, int)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			MonitorNotifyAllEvent record = new MonitorNotifyAllEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
			
			// check values
			Assert.assertEquals("MonitorNotifyAllEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("MonitorNotifyAllEvent.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("MonitorNotifyAllEvent.orderIndex values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getOrderIndex());
			Assert.assertEquals("MonitorNotifyAllEvent.lockId values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getLockId());
	...
	public void testEquality() {
		int i = 0;
		MonitorNotifyAllEvent oneRecord = new MonitorNotifyAllEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
		i = 0;
		MonitorNotifyAllEvent copiedRecord = new MonitorNotifyAllEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
	...
	public void testUnequality() {
		...
		MonitorNotifyAllEvent oneRecord = new MonitorNotifyAllEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
		i = 2;
		MonitorNotifyAllEvent anotherRecord = new MonitorNotifyAllEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()));
