 ...
 ***************************************************************************/
package kicker.test.common.junit.record.flow.trace.concurrency;

import org.junit.Assert;
import org.junit.Test;

import kicker.common.record.controlflow.OperationExecutionRecord;
import kicker.common.record.flow.trace.concurrency.JoinEvent;
 ...
 * @since 1.8
 */
public class TestGeneratedJoinEvent extends AbstractGeneratedKiekerTest {
	...
	/**
	 * Tests {@link JoinEvent#TestJoinEvent(long, long, int, long)}.
	 */
	@Test
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			JoinEvent record = new JoinEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()));
			
			// check values
			Assert.assertEquals("JoinEvent.timestamp values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTimestamp());
			Assert.assertEquals("JoinEvent.traceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getTraceId());
			Assert.assertEquals("JoinEvent.orderIndex values are not equal.", (int) INT_VALUES.get(i % INT_VALUES.size()), record.getOrderIndex());
			Assert.assertEquals("JoinEvent.joinedTraceId values are not equal.", (long) LONG_VALUES.get(i % LONG_VALUES.size()), record.getJoinedTraceId());
	...
	public void testEquality() {
		int i = 0;
		JoinEvent oneRecord = new JoinEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()));
		i = 0;
		JoinEvent copiedRecord = new JoinEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()));
	...
	public void testUnequality() {
		...
		JoinEvent oneRecord = new JoinEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()));
		i = 2;
		JoinEvent anotherRecord = new JoinEvent(LONG_VALUES.get(i % LONG_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()), INT_VALUES.get(i % INT_VALUES.size()), LONG_VALUES.get(i % LONG_VALUES.size()));
