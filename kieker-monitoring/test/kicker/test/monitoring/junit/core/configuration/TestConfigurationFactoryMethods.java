...
package kicker.test.monitoring.junit.core.configuration;
...
import kicker.monitoring.timer.SystemNanoTimer;
...
public class TestConfigurationFactoryMethods extends AbstractKiekerTest {
	...
	static {
		String tempPath;
		try {
			tempPath = new File(TestConfigurationFactoryMethods.class.getResource("/META-INF/kicker.monitoring.test.properties").toURI()).getAbsolutePath();
	...
	public void testCreationDefaultConfiguration() {
		...
		Assert.assertEquals("META-INF/kicker.monitoring.adaptiveMonitoring.conf",
