...
package kicker.test.monitoring.junit.probe.adaptiveMonitoring.mxbean;
...
import kicker.monitoring.sampler.mxbean.ThreadsStatusSampler;
...
public class TestThreadsStatusSampler extends AbstractJVMSamplerTest {
	...
	private static final String LISTNAME = TestThreadsStatusSampler.class.getName();
	private static final String SIGNATURE = SignatureFactory.createJVMThreadsSignature();
	private static final ThreadsStatusSampler SAMPLER = new ThreadsStatusSampler();

	public TestThreadsStatusSampler() {
		super(LISTNAME, SIGNATURE, SAMPLER);
	}

	@Override
	protected void isInstanceOf(final List<IMonitoringRecord> recordList) {
		final boolean isInstanceOf = recordList.get(0) instanceof ThreadsStatusRecord;
